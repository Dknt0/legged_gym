# There is no soft limits in this implementation
# self.body_mass
...
class HumanoidRobotBase(BaseTask):
    def __init__(
        ...
        """
        self.cfg = cfg  # dict of sim config
        self.sim_params = sim_params  # Sim parameter to isaacgym
        self.height_samples = None  # Height samples table for height measurements
        self.debug_viz = False  # Debug visualization
        self.init_done = False  # Initialization flag
        self._parse_cfg(self.cfg)

        # Call create_sim, _create_envs, _create_ground_plane, _create_heightfield, _create_trimesh
    ...
    def step(self, actions):
        ...
        if (
            hasattr(self.cfg, "debug_log")
            and hasattr(self.cfg.debug_log, "debug_log")
            and self.cfg.debug_log.debug_log
        ):
            self.debug_log()
    ...
    def debug_log(self, sub_step: int = 0):
        ...
        if not hasattr(self, "csv_writer"):
            ...
            self.csv_file = open(
                self.cfg.debug_log.save_csv_file_name, mode="w", newline=""
            )
            self.csv_writer = csv.writer(self.csv_file)

            self.csv_writer.writerow(
                self.cfg.debug_log.q_keywords
                + self.cfg.debug_log.dq_keywords
                + self.cfg.debug_log.torque_keywords
                + self.cfg.debug_log.actions_keywords
                + self.cfg.debug_log.lin_vel_keywords
        ...
        cur_q = self.dof_pos[0, :].detach().cpu().tolist()
        cur_dq = self.dof_vel[0, :].detach().cpu().tolist()
        cur_torque = self.torques[0, :].detach().cpu().tolist()
        cur_action = self.actions[0, :].detach().cpu().tolist()
        cur_lin_vel = self.base_lin_vel[0, :].detach().cpu().tolist()
        cur_ang_vel = self.base_ang_vel[0, :].detach().cpu().tolist()
        cur_proj_grav = self.projected_gravity[0, :].detach().cpu().tolist()
        ...
        if self.save_count >= self.cfg.debug_log.max_count:
            self.csv_file.close()
            exit()
