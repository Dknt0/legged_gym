from legged_gym.envs.humanoid.humanoid_base import HumanoidRobotBase
...
class HumanoidRobot(HumanoidRobotBase):
    """
    HumanoidRobot is a class that represents a custom environment for a legged robot.

    Args:
        cfg (HumanoidRobotCfg): Configuration object for the legged robot.
    ...
    def _get_phase(self):
        """Calculates the phase of the gait cycle."""
    ...
    def compute_ref_state(self):
        ...
        if self.cfg.rewards.trajectory_type == "sin":
            # 1. Sin Trajectory
        ...
        elif self.cfg.rewards.trajectory_type == "sin2":
            # 2. Sin2 Trajectory
        ...
        elif self.cfg.rewards.trajectory_type == "sin3":
            # 3. Sin3 Trajectory
            sin_pos_smooth = sin_pos * sin_pos * sin_pos
            sin_pos_smooth_l = -sin_pos_smooth.clone()
            sin_pos_smooth_r = sin_pos_smooth.clone()
            sin_pos_smooth_l[sin_pos_l > 0] = 0
            self.ref_dof_pos[:, 0] = sin_pos_smooth_l * scale_1
    ...
    def step(self, actions):
        ...
        # DKNT added. Action clip
        # for i in range(self.num_actions):
        #     actions[:, i] = torch.clip(
        #         actions[:, i],
        #         self.cfg.control.action_lower_clip[i] / self.cfg.control.action_scale,
        #         self.cfg.control.action_upper_clip[i] / self.cfg.control.action_scale,
        #     )
    ...
    def compute_observations(self):
        ...
        stance_mask = self._get_gait_phase()  # [num_envs, 2]
        contact_mask = (
        ...
        self.privileged_obs_buf = torch.cat(
            (
                ...
                * self.obs_scales.dof_pos,  # 12 + 2
                self.dof_vel * self.obs_scales.dof_vel,  # 12 + 2
                self.actions,  # 12 + 2
                diff,  # 12 + 2
    ...
    def _reward_ankle_torques(self):
        """
        Penalizes the use of torques in the robot's ankle.
        """
